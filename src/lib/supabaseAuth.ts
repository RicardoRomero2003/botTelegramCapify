...
import type { AuthenticatedUser, BotAuthSession } from "./types.js";
...
export function resolveSupabaseEmail(rawIdentifier: string): string {
...
async function parseError(response: Response): Promise<string> {
  ...
  try {
    const payload = JSON.parse(bodyText) as { error_description?: string; error?: string; msg?: string; message?: string; detail?: string };
...
export async function signInWithSupabase(usuario: string, password: string): Promise<BotAuthSession> {
...
export async function refreshSupabaseSession(refreshToken: string): Promise<Pick<BotAuthSession, "accessToken" | "refreshToken">> {
